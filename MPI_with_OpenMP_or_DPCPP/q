...
#==========================================
if [ -z "$1" ]; then
    echo "Missing script argument, Usage: ./q run.sh <num_nodes>"
elif [ ! -f "$1" ]; then
    echo "File $1 does not exist"
else
    if [ -z "$2" ]; then
        nodesstring="nodes=1:ppn=2"
    else
        nodestring=$2
    fi
        
    echo "Job has been submitted to Intel(R) DevCloud with $num_nodes nodes and will execute soon."
    echo ""
    ...
    qsub_id=`qsub -l $nodestring -d . $script`
